...
if [[ -z "$CHAIN" || -z "$TOKEN" || -z "$AMOUNT" || -z "$TRAIL_PCT" ]]; then
    echo "Usage: $0 --chain <chain> --token <addr|alias> --amount <base> --trailpct <pct> [--tokensymbol <name>] [--pollseconds <s>] [--maxiterations <n>] [--dry-run]" >&2
    exit 1
fi
...
echo "  Baseline $BASE_LABEL back : $(fmt8 "$baseline_eth") $BASE_LABEL"
echo "  Initial peak      : $(fmt8 "$peak_eth") $BASE_LABEL"
echo "  Initial floor     : $(fmt8 "$floor_eth") $BASE_LABEL  (peak - $TRAIL_PCT %)"
echo ""
...
while (( iteration < MAX_ITERATIONS )); do
    ...
    pct_from_floor=$(awk "BEGIN { printf \"%.4f\", ($floor_raw > 0) ? ($current_raw - $floor_raw) / $floor_raw * 100 : 0 }")
    ...
    echo -e "${color}[$ts2] $(fmt8 "$eth_back") $BASE_LABEL  peak: $(fmt8 "$peak_eth") $BASE_LABEL  floor: $(fmt8 "$floor_eth") $BASE_LABEL  (${pct_from_peak}% from peak  ${pct_from_floor}% from floor)${RESET}"
    ...
    if awk "BEGIN { exit ($current_raw <= $floor_raw) ? 0 : 1 }"; then
        ...
        echo ""
        echo -e "${RED}Floor breached! $(fmt8 "$eth_back") $BASE_LABEL back  ($gain_pct % vs baseline)${RESET}"
        run_sell "$token_str"
        [[ "$DRY_RUN" == "1" ]] && exit 0
    fi
done
